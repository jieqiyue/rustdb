...
### 5. Join表
将执行节点组织为一棵树的形式。比如说cross join。cross join就是对两张表做笛卡尔积。那么会组织成如下的执行节点的形式：
![Cross Join原理.jpg](src/doc/Cross%20Join%E5%8E%9F%E7%90%86.jpg)

然后自底向上的获取到每一张表的数据，然后再left节点和right节点进行笛卡尔积，然后将结果作为上一层的left节点或者是right节点。最终在最顶层的节点上面
就能够得到最终的结果。
